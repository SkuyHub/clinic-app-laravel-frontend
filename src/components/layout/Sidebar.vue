  ...
  <aside class="fixed bottom-0 left-0 top-0 z-50 flex w-[252px] flex-col bg-clinic-800">
    ...
    <div class="border-b border-white/[0.08] px-5 pb-[18px] pt-[22px]">
      <div class="flex items-center gap-2.5">
        ...
        <span class="font-display text-[19px] font-semibold tracking-[-0.2px] text-white">
          Clinic<em class="text-clinic-300 not-italic">App</em>
        </span>
      </div>
      <div class="mt-1 pl-[46px] text-[10px] font-medium uppercase tracking-[1.8px] text-clinic-300">
        Admin Panel
      </div>
    </div>
    ...
    <nav class="flex-1 overflow-y-auto px-2.5 py-3.5">
      <template v-for="module in menu" :key="module.name">
        ...
        <RouterLink
          ...
          :class="isActive(submodule.name)
            ? 'bg-clinic-500 font-medium text-white shadow-[0_2px_8px_rgba(0,0,0,0.15)]'
            : 'font-normal text-white/60 hover:bg-white/[0.08] hover:text-white'"
        >
