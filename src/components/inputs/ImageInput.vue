  ...
  <BaseInput :label="label" :required="required" :error="error || uploadError">
    <div class="flex items-start gap-4">
      ...
      <div class="flex flex-col gap-2 pt-1">
        <label class="cursor-pointer rounded border border-gray-300 px-3 py-1.5 text-sm text-gray-700 hover:border-gray-400" :class="{ 'pointer-events-none opacity-50': disabled || uploading }">
          {{ uploading ? 'Uploading…' : 'Choose file' }}
          <input type="file" accept="image/*" class="sr-only" :disabled="disabled || uploading" @change="onFileChange" />
        </label>
        <button v-if="previewUrl" type="button" class="text-left text-xs text-danger hover:underline" @click="remove">Remove</button>
