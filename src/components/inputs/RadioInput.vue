...
<template>
  <BaseInput :label="label" :required="required" :error="error">
    <div class="flex flex-wrap gap-5 py-1">
      <label v-for="opt in data" :key="String(opt.id)" class="flex cursor-pointer items-center gap-2 text-sm" :class="{ 'pointer-events-none opacity-50': disabled }">
        <input v-model="normalizedModel" type="radio" :value="opt.id" class="accent-clinic-600" />
        {{ opt.name }}
      </label>
    </div>
  </BaseInput>
